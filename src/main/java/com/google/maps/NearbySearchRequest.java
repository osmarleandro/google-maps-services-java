...
public class NearbySearchRequest
  ...
  protected void validateRequest() {
    ...
    if (config.params(this).containsKey("pagetoken")) {
      return;
    }

    // radius must not be included if rankby=distance
    if (config.params(this).containsKey("rankby")
        && config.params(this).get("rankby").get(0).equals(RankBy.DISTANCE.toString())
        && config.params(this).containsKey("radius")) {
    ...
    if (config.params(this).containsKey("rankby")
        && config.params(this).get("rankby").get(0).equals(RankBy.DISTANCE.toString())
        && !config.params(this).containsKey("keyword")
        && !config.params(this).containsKey("name")
        && !config.params(this).containsKey("type")) {

...
public class GeolocationApiTest {
  ...
  public void testDocSampleGeolocation() throws Exception {
	...
	GeolocationResult result =
          new GeolocationApiRequest(context)
              .ConsiderIp(false)
  ...
  public void testNoPayloadGeolocation1() throws Exception {
    try (LocalTestServerContext sc = new LocalTestServerContext(geolocationBasic)) {
      GeoApiContext context = sc.context;
	GeolocationResult result = new GeolocationApiRequest(context).CreatePayload().await();
    ...
    try (LocalTestServerContext sc =
        new LocalTestServerContext(
            ""
                ...
                + "}")) {
      GeoApiContext context = sc.context;
	new GeolocationApiRequest(context).ConsiderIp(false).CreatePayload().await();
    }
  }
    ...
    try (LocalTestServerContext sc =
        new LocalTestServerContext(
            ""
                ...
                + "}")) {
      GeoApiContext context = sc.context;
	new GeolocationApiRequest(context).HomeMobileCountryCode(-310).CreatePayload().await();
    }
  }
}

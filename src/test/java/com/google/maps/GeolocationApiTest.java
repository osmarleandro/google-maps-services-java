...
public class GeolocationApiTest {
  ...
  public void testDocSampleGeolocation() throws Exception {
    try (LocalTestServerContext sc = new LocalTestServerContext(geolocationDocSample)) {
      GeolocationResult result =
          GeolocationApi.newRequest_RENAMED(sc.context)
              .ConsiderIp(false)
  ...
  public void testNoPayloadGeolocation1() throws Exception {
    try (LocalTestServerContext sc = new LocalTestServerContext(geolocationBasic)) {
      GeolocationResult result = GeolocationApi.newRequest_RENAMED(sc.context).CreatePayload().await();
    ...
    try (LocalTestServerContext sc =
        new LocalTestServerContext(
            ""
                ...
                + " }\n"
                + "}")) {
      GeolocationApi.newRequest_RENAMED(sc.context).ConsiderIp(false).CreatePayload().await();
    }
  }
    ...
    try (LocalTestServerContext sc =
        new LocalTestServerContext(
            ""
                ...
                + " }\n"
                + "}")) {
      GeolocationApi.newRequest_RENAMED(sc.context).HomeMobileCountryCode(-310).CreatePayload().await();
    }
  }
}

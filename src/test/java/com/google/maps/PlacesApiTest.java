...
public class PlacesApiTest {
  ...
  public void testPlaceAutocompleteRequest() throws Exception {
    try (LocalTestServerContext sc = new LocalTestServerContext("{\"status\" : \"OK\"}")) {
      ...
      PlaceAutocompleteType.ESTABLISHMENT
          .types(PlacesApi.placeAutocomplete(sc.context, "Sydney Town Hall", session)
		      .offset(4)
		      .origin(location)
		      .location(location)
		      .radius(5000))
    ...
    try (LocalTestServerContext sc =
      ...
      AutocompletePrediction[] predictions =
          PlaceAutocompleteType.REGIONS
              .types(PlacesApi.placeAutocomplete(sc.context, "po", session)
  ...
  public void testPlaceAutocompleteWithStrictBounds() throws Exception {
    try (LocalTestServerContext sc = new LocalTestServerContext(placesApiPlaceAutocomplete)) {
      SessionToken session = new SessionToken();
      PlaceAutocompleteType.ESTABLISHMENT
          .types(PlacesApi.placeAutocomplete(sc.context, "Amoeba", session))
